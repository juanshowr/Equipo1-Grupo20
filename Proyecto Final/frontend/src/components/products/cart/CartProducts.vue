    ...
    <div class="row mt-5">
        ...
        <div class="row">
          <template v-if="cartProducts.length > 0">
            ...
            >
              <div class="card mb-4 shadow-sm" style="height: 410px">
                ...
                />
                <div class="card-body">
                  <h6 class="card-text">{{ item.producto.imagen }}.</h6>
                  <div
          ...
          <template v-else>
            <div class="error-template container">
              ...
              <div class="error-details">Tu carrito está vacío</div>
              <div class="error-actions">
                <a class="btn text-white">
                  <span class="glyphicon glyphicon-envelope"></span>
                  <router-link to="/"
                    >Llévame a la página de productos</router-link
                  >
                </a>
              </div>
            </div>
      ...
      <div class="col-4">
        ...
        <ul class="list-group mb-3">
          <router-link to="/" class="btn btn-primary mt-2 text-white"
            >Continúa comprando</router-link
          >
...
export default {
  ...
  methods: {
    ...
    removeProductCart(producto) {
      const productos = JSON.parse(localStorage.getItem("iki-cart"));
      for (let i = 0; i < products.length; i++) {
        if (productos[i]._id === product._id) {
          productos.splice(i, 1);
        }
      }
      this.SET_CART_PRODUCTS(productos);
      localStorage.setItem("iki-cart", JSON.stringify(productos));
